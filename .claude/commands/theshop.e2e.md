---
description: The full E2E gate for one feature in a single context — classify every acceptance criterion, write the browser journey, start the local stack, run it, repair its locators, and report. Supersedes /theshop.verify.
...
Own the entire end-to-end proof for **The Shop** in this context:

`read -> classify -> write -> gate -> start env -> run -> repair -> re-run -> report`

Never invoke a sub-agent or the Task tool. The reason this command exists is that browser code cannot be validated by compiling it: locators, waits, dialog timing, and auth state are only knowable by execution. Writing the journey and running it therefore belong in one context, with one feedback loop.

This command supersedes `/theshop.verify` and writes the same `5. Verify` ledger row, so `ship-ready` keeps working unchanged. If both commands exist, run this one.
...
This is the step that makes the gate honest, and it happens **before** you write a single test. A journey that silently proves 11 of 32 ACs is indistinguishable from one that proves all 32 unless the other 21 are accounted for. So account for them.
...
Leaving that unresolvable journey on disk is intended and safe: the gate's hook check fails **only** for the feature under test (its manifest journeys and the page objects they use) and merely warns about other features' pending hooks, so your halted journey will not block anyone else's `/theshop.e2e`. It will keep blocking *this* feature until the Web layer lands the hook — which is the point.

## 5. Static gates — everything knowable without a browser

```powershell
pwsh -NoProfile -ExecutionPolicy Bypass -File .claude/scripts/check-sdd-gates.ps1 e2e -Feature $ARGUMENTS
...
Do not start the environment while a static gate is red. Every failure caught here costs seconds; the same failure caught after `supabase db reset` costs minutes.
...
The load-bearing rule: **when you cannot tell a locator defect from a product defect, it is a product defect.** Failing closed costs one investigation; failing open ships the bug the gate existed to catch.
...
**If you edited any test file in this step, re-run the `e2e` gate once before reporting:**

```powershell
pwsh -NoProfile -ExecutionPolicy Bypass -File .claude/scripts/check-sdd-gates.ps1 e2e -Feature $ARGUMENTS
```

Step 5's run proved the journey *as first written*; a repair can introduce a locator whose hook does not exist or rename a method off its `AC{n}_` mapping, and nothing else in this flow would catch either. If this run is red, the repair is not finished — fix it or report the failure, but never report VERIFIED over a gate you invalidated.
...
Then update `.specs/$ARGUMENTS/status.md` — the **`5. Verify`** row (that name is what `ship-ready` scans; keep it even though this command is not `/theshop.verify`):
...
Refresh `Last updated` and point `Next step` at `/theshop.review $ARGUMENTS`. If `status.md` is missing, read only the status-template section of `.claude/skills/theshop.spec/SKILL.md`, create the tracker, then apply the update.
