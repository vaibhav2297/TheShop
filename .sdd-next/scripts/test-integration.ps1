...
$canonical = Join-Path $root '.sdd-next/theshop-build/SKILL.md'
$codex = Join-Path $root '.agents/skills/theshop-build/SKILL.md'
$claude = Join-Path $root '.claude/skills/theshop-build/SKILL.md'
$hook = Join-Path $root '.sdd-next/scripts/claude-design-hook.ps1'
$checker = Join-Path $root '.sdd-next/scripts/check-design-rules.ps1'

foreach ($path in @($canonical, $codex, $claude, $hook, $checker, (Join-Path $root 'AGENTS.md'), (Join-Path $root 'CLAUDE.md'))) {
    Assert-True (Test-Path -LiteralPath $path -PathType Leaf) "Missing required Next file: $path"
}

foreach ($path in @($codex, $claude)) {
    $content = Get-Content -LiteralPath $path -Raw
    Assert-True ($content -match '\.sdd-next/theshop-build/SKILL\.md') "Native entry does not load canonical skill: $path"
    Assert-True ($content -notmatch '(?m)^.*\.sdd/(?!-next)') "Native entry references retired SDD: $path"
}
...
Assert-True (($commands -match '\.sdd-next/scripts/claude-design-hook\.ps1').Count -gt 0) 'Claude settings do not register Next design hook.'
Assert-True (($commands -match '\.sdd/').Count -eq 0) 'Claude settings still register retired SDD hook.'

Assert-ParserClean $hook
Assert-ParserClean $checker
Assert-ParserClean $PSCommandPath

$fixture = Join-Path $root 'src/TheShop.Web/.sdd-next-design-fixture.razor'
try {
...
$legacyMatches = Get-ChildItem -LiteralPath $existingActivePaths -Recurse -File | Select-String -Pattern '\.sdd/(?!-next)|\.sdd\\(?!-next)|theshop-(clarify|constitution|document|e2e|execute|implement|plan|resolve|review|ship|spec|start|test|test-merged|verify)|sdd-cavecrew-reviewer' -AllMatches
if ($legacyMatches) { throw "Active legacy reference remains: $($legacyMatches[0].Path):$($legacyMatches[0].LineNumber)" }

Assert-True (-not (Test-Path -LiteralPath (Join-Path $root '.sdd'))) 'Retired .sdd directory remains.'
Write-Output 'SDD Next integration checks passed.'
