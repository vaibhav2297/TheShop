...
description: "Implement four layers in one context with plan, scope/build gates, migration, Figma parity, and ledger. Explicit alternative to $theshop-implement; no subagents."
---

<!-- Generated from .sdd/skills/theshop-execute/SKILL.md. Edit shared source; run sync-adapters.ps1. -->
...
Execute `$theshop-implement` contract alone: four layers, same gates and report, one context, no subagents. Preserve behavior equivalence for A/B comparison.

## Scope

Include required implementation tests under project rule 6; separate Test stage still owns full acceptance coverage. XML documentation remains manually invoked through `$theshop-document`.
...
3. Unfamiliar MudBlazor component → `capability:mudblazor.get_component_detail`.
...
2. `$theshop-test {feature}` → 3. `$theshop-e2e {feature}` (user-facing) → 4. `$theshop-review {feature}` → 5. `$theshop-document` when final.
```
