...
<!-- Generated from .sdd/skills/theshop-constitution/SKILL.md. Edit shared source; run sync-adapters.ps1. -->
...
**Organising principle differs by layer.** `Application` is grouped by **business feature** (vertical slices — `Features/Cart/`, `Features/Checkout/`; Rule 9). `Infrastructure` is grouped by **technical concern / external system** — `Persistence/` (the database), `Auth/`, `Payments/`, `Email/`, `Storage/`. There is **no** `Infrastructure/{Feature}/`: a feature's infra is scattered into the concern folders (e.g. Cart → `Persistence/Records/CartRecord.cs`, `Persistence/Mappers/CartMapper.cs`, `Persistence/Repositories/SupabaseCartRepository.cs`). The `Records/Mappers/Repositories` trio lives **only** under `Persistence/`; `Auth/Payments/Email/Storage/` are flat adapter folders. The `Persistence/` **root** holds only cross-cutting DB plumbing (`SupabaseClientFactory`, shared query extensions); anything feature-specific goes in a subfolder, and anything that is not the database concern (e.g. a Storage-bucket adapter) goes in its own concern folder — never loose in `Persistence/`.
