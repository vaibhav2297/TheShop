...
<!-- Generated from .sdd/skills/theshop-document/SKILL.md. Edit shared source; run sync-adapters.ps1. -->
...
Current diff; optional feature name in `{arguments}` controls ledger update. Manual step after `$theshop-test`, `$theshop-verify`, and `$theshop-review`. `$theshop-implement` never runs documenter. Document settled code.
