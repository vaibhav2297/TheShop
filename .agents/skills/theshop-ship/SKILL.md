...
<!-- Generated from .sdd/skills/theshop-ship/SKILL.md. Edit shared source; run sync-adapters.ps1. -->
...
Land finished feature on `dev` after `$theshop-review` approves diff. Sequence: update tracker, commit, push, open PR, merge, delete branch, return to `dev`. Confirm remote mutations as specified below.
...
Use same `{feature-name}` slug for `.specs/{feature-name}/` and branch from `$theshop-start`.
...
Step 1 ships tracker inside PR; no later tracker edit remains. Report actual branch and working-tree state before next `$theshop-start`.
