<!-- Generated from .sdd/contracts/project.md. -->
...
The two git bookends (**Start** / **Ship**) wrap the document pipeline: `$theshop-start` puts you on a `feature/{feature}` branch before any artifact is written; `$theshop-ship` lands that branch on `dev` once the feature is reviewed. Both require explicit invocation (never auto-invoked) and every remote-mutating action in `$theshop-ship` — push, PR, merge, delete — is gated behind an explicit confirmation.
...
## Portable SDD runtime

Read `.sdd/contracts/execution.md` before any SDD workflow. Use the active adapter under `.sdd/adapters/`. Canonical workflows and references live in `.sdd/skills/`; native definitions are generated. Both runtimes share `.specs/`. Edit shared sources and run `pwsh -NoProfile -File .sdd/scripts/sync-adapters.ps1`; validate with `test-portability.ps1`.
