...
/// (<see cref="OptionTypes"/>), and generated variants (<see cref="Variants"/>) as a single
/// aggregate; every cross-child invariant — one primary image, variants as the cartesian
/// product of option values, a pin resolving only to this product's own gallery, publish
/// completeness — is enforced here rather than by a handler.
/// </summary>
public sealed class Product
{
    ...
    /// <summary>
    /// Creates a new <see cref="Product"/> with an empty gallery, no option types, and no
    /// variants. Gallery, option types, and variants are attached afterward via
    /// <see cref="SetGallery"/>, <see cref="SetOptionTypes"/>, and
    /// <see cref="ApplyVariantConfiguration"/>.
    /// </summary>
    ...
    /// <exception cref="ProductDescriptionTooLongException">The trimmed description exceeds 2000 characters.</exception>
        ...
        DateTimeOffset? createdAt = null)
    {
        ...
        return new Product(
            Guid.NewGuid(), trimmedName, trimmedDescription, imageUrl, sku, pricing,
    ...
    /// invariants, including its gallery, option types, and variants. When the caller does not
    /// embed <paramref name="variants"/> (a read-optimized list projection), pass
    /// <paramref name="hasVariants"/> / <paramref name="minVariantPrice"/> /
    ...
    /// <summary>
    /// Updates the product's name, description, category, and brand, re-validating the same
    /// invariants <see cref="Create"/> enforces.
    /// </summary>
    /// <exception cref="ProductNameRequiredException">The trimmed name is empty.</exception>
    /// <exception cref="ProductNameTooLongException">The trimmed name exceeds 150 characters.</exception>
    /// <exception cref="ProductDescriptionTooLongException">The trimmed description exceeds 2000 characters.</exception>
    public void UpdateDetails(string name, string description, Category category, Brand brand)
    {
