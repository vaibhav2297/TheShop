...
/// <summary>
/// The manage-categories admin list: a paged, searchable, filterable, sortable table with row
...
/// (<see cref="CategoryQueryState"/>). Requires a signed-in user via
/// <c>Pages/Admin/_Imports.razor</c>, and is gated on <c>categories.view</c> within the page
/// itself.
/// </summary>
[Route(Routes.Admin.ManageCategories)]
[AuthorizePermission("categories.view")]
public partial class ManageCategories : QueryStatePageBase<CategoryQueryState>
{
    ...
    /// <see cref="CategorySortCatalogue"/>, which also owns the matching URL slugs and the
    /// default — so an added order reaches the picker, the link, and the fallback together.
    /// </summary>
    private static readonly IReadOnlyList<(CategorySortOption Value, string LabelKey)> SortOptions =
        CategorySortCatalogue.Instance.Picker;
    ...
    [
        BusyKeys.Categories.CategoryStatus,
        BusyKeys.Categories.DeleteCategories,
    ];
    ...
    private Paginator<CategoryListItemDto> _categories = default!;
    private string? _search;
    private CategoryStatusFilter? _status;
    private CategorySortOption _sort = CategorySortCatalogue.Instance.Default;
    ...
    /// </summary>
    private HashSet<CategoryListItemDto> _selectedItems = [];

    /// <summary>
    /// The identifiers of rows whose deletion was blocked because the categories are in use. This
    /// is page-owned UI state and is cleared whenever query state is applied.
    /// </summary>
    ...
    {
        Breadcrumbs.Set(BreadcrumbTrail.Admin().Current(Strings.ManageCategories_Heading));
        _categories = new Paginator<CategoryListItemDto>(FetchCategoriesAsync, PageSize);
    }

    /// <summary>
    /// Applies a manage-categories state — snapshot the criteria, clear the transient selection
    /// (RULE-14), then load the requested page. Runs on first render and on every URL change.
    /// </summary>
    protected override Task ApplyStateAsync(CategoryQueryState state, CancellationToken ct)
    {
        ...
        return BusyState.RunAsync(BusyKeys.Categories.ManageList, () => _categories.GoToAsync(state.Page, ct));
    }

    private CategoryQueryState BuildState() => new(_search, _status, _sort, _categories.Page);

    private async Task<PagedResult<CategoryListItemDto>> FetchCategoriesAsync(
        PaginationRequest request, CancellationToken ct)
    {
        var query = new GetCategoriesPageQuery(_search, _status, _sort, request);
    ...
    private Task OnSortChangedAsync(CategorySortOption sort)
    {
        _sort = sort;
        return PushStateAsync(BuildState() with { Page = 1 });
    }
    ...
    private void OnSelectedItemsChanged(HashSet<CategoryListItemDto> items) => _selectedItems = items;
    ...
    private async Task OnStatusToggledAsync(CategoryListItemDto category, bool activate)
    {
        if (activate)
        {
            await ApplyStatusChangeAsync([category.Id], true);
            return;
        }
    ...
    private async Task BulkActivateAsync() =>
        await ApplyStatusChangeAsync([.. _selectedItems.Select(c => c.Id)], true);

    private async Task BulkDeactivateAsync()
    {
        var ids = _selectedItems.Select(c => c.Id).ToList();
        if (ids.Count == 0)
            return;
    ...
    private async Task ApplyStatusChangeAsync(IReadOnlyList<Guid> ids, bool isActive)
    {
        ...
        await BusyState.RunAsync(BusyKeys.Categories.CategoryStatus, async () =>
        {
            try
            {
    ...
    private async Task DeleteSingleAsync(CategoryListItemDto category)
    {
        var confirmed = await ConfirmAsync(
    ...
    private async Task ApplyDeleteAsync(IReadOnlyList<Guid> ids)
    {
        ...
        await BusyState.RunAsync(BusyKeys.Categories.DeleteCategories, async () =>
        {
            try
            {
    ...
    /// <summary>
    /// Handles a single-category deletion result, reporting success or identifying the category
    /// whose deletion was blocked because it is in use.
    /// </summary>
    private void HandleSingleDeleteOutcome(CategoryDeletionOutcomeDto outcome)
    {
    ...
    /// <summary>
    /// Handles a bulk-deletion result, reporting aggregate counts and keeping blocked categories
    /// selected so the table can identify them with in-use indicators.
    /// </summary>
    private void HandleBulkDeleteOutcome(int requestedCount, CategoryDeletionOutcomeDto outcome)
    {
        ...
        // would drop the deleted ones), so they can be looked up by id to rebuild the table's
        // selection.
        var blockedIds = outcome.Blocked.Select(c => c.Id).ToHashSet();
    ...
    {
        CategoryStatusFilter.Active => StatusActiveValue,
    ...
    /// filter.
    /// </summary>
    private static CategoryStatusFilter? ParseStatusValue(string? value) => value switch
    {
        StatusActiveValue => CategoryStatusFilter.Active,
        StatusInactiveValue => CategoryStatusFilter.Inactive,
        _ => null,
    };
}
