...
/// <summary>
/// Validates <see cref="UpdateProductCommand"/>. Identical field-shape rules to
/// <c>CreateProductCommandValidator</c> (plan Section 9), plus <see cref="UpdateProductCommand.Id"/>
/// and <see cref="UpdateProductCommand.RowVersion"/> presence.
/// </summary>
public sealed class UpdateProductCommandValidator : AbstractValidator<UpdateProductCommand>
{
    ...
    {
        RuleFor(x => x.Id).NotEmpty().WithMessage(ProductErrorKeys.NotFound);
        RuleFor(x => x.RowVersion).NotEmpty().WithMessage(ProductErrorKeys.ModifiedElsewhere);
    ...
    /// </summary>
    private static void ValidateDescription(string? description, ValidationContext<UpdateProductCommand> context)
    {
    ...
    private static void ValidateSpecifications(
        IReadOnlyList<SpecificationInput> specifications, ValidationContext<UpdateProductCommand> context)
    {
    ...
    private static ValidationFailure RowFailure(string errorKey, IReadOnlyList<int> positions) =>
        new(nameof(UpdateProductCommand.Specifications), errorKey)
        {
