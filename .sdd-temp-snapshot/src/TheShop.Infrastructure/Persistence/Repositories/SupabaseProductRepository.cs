...
public sealed class SupabaseProductRepository(Supabase.Client client, IFileStorage fileStorage) : IProductRepository
{
    ...
    // Matches the DB constraint names from migrations 0029/0030 — the security boundary behind
    // the C# grammar/length fast feedback (plan Decision 4).
    private const string DescriptionMarkupConstraint = "products_description_markup_allowed";
    private const string DescriptionSizeConstraint = "products_description_size";
    private const string SpecificationNameUniqueIndex = "ux_product_specifications_name";
    ...
    public async Task<(Product Product, string RowVersion)?> GetForEditAsync(Guid id, CancellationToken ct)
    {
        ...
        var specificationRecords = await client.From<ProductSpecificationRecord>().Where(x => x.ProductId == id).Get(ct);
        ...
        var optionTypes = optionTypeRecords.Models
            ...
            .ToList();
        var specifications = specificationRecords.Models.OrderBy(s => s.Position).Select(s => s.ToDomain()).ToList();
    ...
    public async Task<Result<string>> AddAsync(Product product, CancellationToken ct)
    {
        ...
        catch (Exception ex) when (IsDescriptionMarkupViolation(ex))
        {
            return Result.Fail<string>(ProductErrorKeys.DescriptionUnsupportedContent);
        }
        catch (Exception ex) when (IsDescriptionSizeViolation(ex))
        {
            return Result.Fail<string>(ProductErrorKeys.DescriptionTooLarge);
        }
        catch (Exception ex) when (IsSpecificationNameUniqueViolation(ex))
        {
            return Result.Fail<string>(ProductErrorKeys.SpecificationNameDuplicated);
        }
    }
    ...
    private static bool IsDescriptionMarkupViolation(Exception ex) =>
        ex.Message.Contains(DescriptionMarkupConstraint, StringComparison.OrdinalIgnoreCase);

    private static bool IsDescriptionSizeViolation(Exception ex) =>
        ex.Message.Contains(DescriptionSizeConstraint, StringComparison.OrdinalIgnoreCase);

    private static bool IsSpecificationNameUniqueViolation(Exception ex) =>
        ex.Message.Contains(SpecificationNameUniqueIndex, StringComparison.OrdinalIgnoreCase);
}
