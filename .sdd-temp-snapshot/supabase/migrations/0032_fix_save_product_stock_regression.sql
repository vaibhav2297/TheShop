-- ============================================================================
-- 0032_fix_save_product_stock_regression
--
-- 0031_save_product_specifications redefined save_product(payload) from a
-- stale copy predating 0026_remove_product_stock, reintroducing
-- stock_quantity reads/writes against products and product_variants after
-- both columns were dropped. Restores the stock-free body from 0026 while
-- keeping 0031's specifications insert. Nothing else changes.
-- ============================================================================
...
BEGIN
    ...
    IF v_mode = 'create' THEN
        INSERT INTO products (
            ...
            original_price, sale_price, is_published,
    ...
    ELSE
        ...
        DELETE FROM product_specifications WHERE product_id = v_product_id;
    ...
    -- Specifications --------------------------------------------------------
    FOR v_specification IN SELECT * FROM jsonb_array_elements(COALESCE(payload->'specifications', '[]'::jsonb))
    LOOP
        INSERT INTO product_specifications (id, product_id, name, value, position)
        VALUES (
            COALESCE(NULLIF(v_specification->>'id', '')::uuid, gen_random_uuid()),
            v_product_id,
            v_specification->>'name',
            v_specification->>'value',
            (v_specification->>'position')::integer
        );
    END LOOP;
    ...
    LOOP
        ...
        BEGIN
            INSERT INTO product_variants (
                id, product_id, sku, original_price, sale_price,
