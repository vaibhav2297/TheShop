...
public sealed class ProductFormPage(IPage page)
{
    ...
    // ---- Content card: description (ShopRichTextEditor, .specs/product-description) -----------

    /// <summary>
    /// The Quill-backed rich-text editor's own editable region. Quill mounts on an inner div it
    /// appends to <c>ShopRichTextEditor</c>'s host (class <c>shop-rich-text-editor</c>), so it is
    /// addressed by that project class plus Quill's own <c>.ql-editor</c> class, the same
    /// established pattern as the gallery's <c>.shop-image-upload</c> locators above. Playwright's
    /// <c>FillAsync</c> supports a <c>contenteditable</c> region directly.
    /// </summary>
    public ILocator DescriptionEditor => Page.Locator(".shop-rich-text-editor .ql-editor");

    public ILocator DescriptionFormattingRemovedNotice =>
        Page.GetByText(Strings.AddProduct_DescriptionFormattingRemoved, new() { Exact = true });

    public Task FillDescriptionAsync(string description) => DescriptionEditor.FillAsync(description);

    /// <summary>
    /// Quill's own <c>snow</c> toolbar supplies formatting controls (plan Decision 2 waiver — its
    /// buttons carry no resx-driven accessible name, so unlike every other locator in this file
    /// these are addressed by Quill's own CSS classes rather than <c>Strings</c>/ARIA name).
    /// </summary>
    private ILocator ToolbarBoldButton => Page.Locator(".shop-rich-text-editor .ql-toolbar button.ql-bold");

    private ILocator ToolbarItalicButton => Page.Locator(".shop-rich-text-editor .ql-toolbar button.ql-italic");

    private ILocator ToolbarOrderedListButton =>
        Page.Locator(".shop-rich-text-editor .ql-toolbar button.ql-list[value='ordered']");

    private ILocator ToolbarBulletListButton =>
        Page.Locator(".shop-rich-text-editor .ql-toolbar button.ql-list[value='bullet']");

    private ILocator ToolbarHeaderPickerLabel =>
        Page.Locator(".shop-rich-text-editor .ql-toolbar .ql-header .ql-picker-label");

    private ILocator ToolbarLinkButton => Page.Locator(".shop-rich-text-editor .ql-toolbar button.ql-link");

    /// <summary>Toggles bold on the current selection or, with none, on text typed next.</summary>
    public Task ApplyBoldAsync() => ToolbarBoldButton.ClickAsync();

    /// <summary>Toggles italic on the current selection or, with none, on text typed next.</summary>
    public Task ApplyItalicAsync() => ToolbarItalicButton.ClickAsync();

    /// <summary>Toggles a numbered list on the current line.</summary>
    public Task ApplyOrderedListAsync() => ToolbarOrderedListButton.ClickAsync();

    /// <summary>Toggles a bulleted list on the current line.</summary>
    public Task ApplyBulletListAsync() => ToolbarBulletListButton.ClickAsync();

    /// <summary>Sets the current line's heading level (1-6) via Quill's own header picker.</summary>
    public async Task ApplyHeadingAsync(int level)
    {
        await ToolbarHeaderPickerLabel.ClickAsync();
        await Page.Locator($".shop-rich-text-editor .ql-toolbar .ql-header .ql-picker-item[data-value='{level}']")
            .ClickAsync();
    }

    /// <summary>
    /// Converts the current selection into a link via Quill's own (unlocalized) link tooltip —
    /// select the text with the keyboard or mouse before calling this.
    /// </summary>
    public async Task ApplyLinkAsync(string url)
    {
        await ToolbarLinkButton.ClickAsync();
        var input = Page.Locator(".shop-rich-text-editor .ql-tooltip input[type='text']");
        await input.WaitForAsync(new() { Timeout = 15_000 });
        await input.FillAsync(url);
        await input.PressAsync("Enter");
    }

    /// <summary>
    /// Dispatches a synthetic <c>paste</c> event carrying the given HTML on the editor region,
    /// bypassing the OS clipboard entirely — the standard, reliable way to drive a paste handler
    /// in a headless-unfriendly, cross-platform way. Triggers both Quill's own clipboard matchers
    /// (which drop unregistered formats) and <c>shop-rich-text-editor.js</c>'s own paste listener
    /// (FR-7/AC-10's "formatting removed" notice).
    /// </summary>
    public Task PasteIntoDescriptionAsync(string html) => DescriptionEditor.EvaluateAsync(
        "(el, html) => { " +
    ...
    // ---- Content card: specifications (ProductContentCard, .specs/product-description) ---------

    public ILocator SpecificationEmptyStateTitle =>
        Page.GetByText(Strings.ProductSpecifications_EmptyStateTitle, new() { Exact = true });

    public ILocator AddSpecificationButton =>
        Page.GetByRole(AriaRole.Button, new() { Name = Strings.ProductSpecifications_AddRow });

    public Task AddSpecificationRowAsync() => AddSpecificationButton.ClickAsync();

    /// <summary>A specification row's name field, by creation order (0-based) — the placeholder is shared across every row.</summary>
    public ILocator SpecificationNameField(int index) =>
        Page.GetByPlaceholder(Strings.ProductSpecifications_NameLabel).Nth(index);

    /// <summary>A specification row's value field, by creation order (0-based) — the placeholder is shared across every row.</summary>
    public ILocator SpecificationValueField(int index) =>
        Page.GetByPlaceholder(Strings.ProductSpecifications_ValueLabel).Nth(index);

    public ILocator SpecificationRemoveButton(int index) =>
        Page.GetByRole(AriaRole.Button, new() { Name = Strings.ProductSpecifications_RemoveRow }).Nth(index);

    public Task FillSpecificationNameAsync(int index, string name) => FillAndBlurAsync(SpecificationNameField(index), name);
    public Task FillSpecificationValueAsync(int index, string value) => FillAndBlurAsync(SpecificationValueField(index), value);
    public Task RemoveSpecificationRowAsync(int index) => SpecificationRemoveButton(index).ClickAsync();
}
