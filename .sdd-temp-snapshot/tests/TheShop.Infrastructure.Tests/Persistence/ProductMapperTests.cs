...
public class ProductMapperTests
{
    ...
    // =========================================================================
    // product-description — Description rehydration skips grammar/length checks (Decision 8, AC-3)
    // =========================================================================

    [Fact]
    [Trait("Feature", "product-description")]
    public void ToDomain_MapsDescriptionHtmlFromTheRecord()
    {
        var record = BuildRecord(imagePath: null);
        record.Description = "<p>Long-lasting.</p>";

        var product = record.ToDomain(ResolvePublicUrl);

        product.Description.Html.Should().Be("<p>Long-lasting.</p>");
    }

    [Fact]
    [Trait("Feature", "product-description")]
    public void ToDomain_WithALegacyStoredDescriptionOutsideTheGrammar_DoesNotThrow()
    {
        // A stored row must always load even if it predates the Decision 3 grammar or a bound
        // has since tightened — Product.Rehydrate (Decision 8) skips both checks.
        var record = BuildRecord(imagePath: null);
        record.Description = "<span>Unsupported but already stored</span>";
