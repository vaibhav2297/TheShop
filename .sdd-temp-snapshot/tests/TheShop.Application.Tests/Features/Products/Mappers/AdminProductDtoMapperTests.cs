...
public class AdminProductDtoMapperTests
{
    ...
    private static Product BuildProductWithVariants(params decimal?[] variantPrices)
    {
        var product = Product.Create(
            "Elf Bar BC5000", ProductDescription.Create("A long-lasting disposable vape."), null,
    ...
    // =========================================================================
    // ToAdminDto — description and specifications (product-description AC-1, AC-2)
    // =========================================================================

    [Fact]
    [Trait("Feature", "product-description")]
    public void ToAdminDto_MapsTheDescriptionHtmlVerbatim()
    {
        var product = Product.Create(
            "Elf Bar BC5000", ProductDescription.Create("<p>Long-lasting.</p>"), null,
            Sku.Create("ELF-BC5000"), null, false, ExampleCategory(), ExampleBrand());

        var dto = AdminProductDtoMapper.ToAdminDto(product, _fileStorage, "row-version-1");

        dto.Description.Should().Be("<p>Long-lasting.</p>");
    }

    [Fact]
    [Trait("Feature", "product-description")]
    public void ToAdminDto_MapsSpecificationsOrderedByPosition()
    {
        var product = Product.Create(
            "Elf Bar BC5000", ProductDescription.Create(string.Empty), null,
            Sku.Create("ELF-BC5000"), null, false, ExampleCategory(), ExampleBrand());
        ...
        dto.Specifications.Should().SatisfyRespectively(
            first =>
            {
                first.Name.Should().Be("Material");
                first.Value.Should().Be("Stainless steel");
                first.Position.Should().Be(0);
            },
            second =>
            {
                second.Name.Should().Be("Capacity");
                second.Value.Should().Be("750 ml");
...
// product-description AC-1/AC-2 (reopening shows saved description/rows in saved order): ToAdminDto_MapsTheDescriptionHtmlVerbatim,
//        ToAdminDto_MapsSpecificationsOrderedByPosition
