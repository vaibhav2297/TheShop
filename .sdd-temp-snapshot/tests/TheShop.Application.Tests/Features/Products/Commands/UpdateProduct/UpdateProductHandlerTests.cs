...
public class UpdateProductHandlerTests
{
    ...
    private Product StoredProduct() =>
        Product.Rehydrate(
            _productId, "Elf Bar BC5000", ProductDescription.Rehydrate("A long-lasting disposable vape."), null,
    ...
    private UpdateProductCommand Command(
        ...
        new(id ?? _productId, rowVersion, name, "A long-lasting disposable vape.", "ELF-BC5000",
            _categoryId, _brandId, originalPrice, null, false, gallery ?? [], [], specifications ?? [], []);
    ...
    // =========================================================================
    // Specifications (product-description RULE-3, RULE-4, AC-2)
    // =========================================================================

    [Fact]
    [Trait("Feature", "product-description")]
    public async Task Handle_WithEditedSpecificationRows_ReplacesThemAndLeavesUnrelatedDetailsUnchanged()
    {
        _products.UpdateAsync(Arg.Any<Product>(), Arg.Any<string>(), Arg.Any<CancellationToken>())
            .Returns(Result.Ok("row-version-2"));

        var result = await CreateSut().Handle(Command(specifications: [
            new SpecificationInput(null, "Material", "Aluminum", 0),
        ]), CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        result.Value.Specifications.Should().ContainSingle(s => s.Name == "Material" && s.Value == "Aluminum");
        result.Value.Name.Should().Be("Elf Bar BC5000", "editing specifications must not touch unrelated saved details");
    }
    ...
    public async Task Handle_WithDuplicateSpecificationNames_FailsWithSpecificationNameDuplicated()
    {
        ...
        await _products.DidNotReceive().UpdateAsync(Arg.Any<Product>(), Arg.Any<string>(), Arg.Any<CancellationToken>());
    }
...
// product-description AC-2 (edit specifications; unrelated details unchanged): Handle_WithEditedSpecificationRows_ReplacesThemAndLeavesUnrelatedDetailsUnchanged
// product-description AC-8/AC-15 (duplicate names rejected, saved product unchanged): Handle_WithDuplicateSpecificationNames_FailsWithSpecificationNameDuplicated
