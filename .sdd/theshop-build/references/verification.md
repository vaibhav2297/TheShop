...
4. Run isolated design checker against changed production C#/Razor files: `pwsh -NoProfile -File .sdd/scripts/check-design-rules.ps1 -Path <paths>`. In PowerShell pass multiple paths as an array within the same invocation. No relevant files means not applicable; never pass nonexistent paths.
...
6. Review changed code against relevant constitution and references. Mechanical checker covers only a subset. Verify English resource text, format placeholders, public contracts, permissions, and required member tests. Apply independent review when security reference requires it.
