...
> User: `{{command:theshop-resolve}} add-to-cart`
>
> Claude: reads `.specs/add-to-cart/plan.md` → finds 1 open question, 2 assumptions, 1 risk → asks one technical question per item (defaults marked *Recommended*) → folds each decision into Sections 4/5/7/10 → dispositions the risk (mitigate or accept) → empties Section 11 → sets Status `Resolved` → "Resolved 4 items… you're clear to run `{{command:theshop-implement}} add-to-cart`."

**Example 2 — no feature name provided:**

> User: `{{command:theshop-resolve}}`
>
...
> User: `{{command:theshop-resolve}} checkout`
>
...
> User: `{{command:theshop-resolve}} wishlist`
>
> Claude: "I couldn't find a plan at `.specs/wishlist/plan.md`. I resolve existing plans — create one first with `{{command:theshop-plan}} wishlist`, then re-invoke me."
