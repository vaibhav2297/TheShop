...
>
> Skill: "I couldn't find a spec at `.specs/nonexistent-feature/spec.md`. I generate implementation plans from specs — please create the spec first (the `{{command:theshop-spec}}` skill helps) and re-invoke me."
...
> User: `{{command:theshop-plan}} user-authentication --figma https://www.figma.com/file/XXXXX/TheShop?node-id=42-100`
>
...
> User: `{{command:theshop-plan}} user-authentication`
>
...
> User: `{{command:theshop-plan}} add-to-cart --desc reuse the existing CartState store; enforce a unique (cart_id, product_id) index to handle double-add --figma 123:456`
>
