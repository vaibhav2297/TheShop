...
`{feature}` has no Web-layer surface (no Phase 4, no Figma references, no `.razor` files), so there is nothing to drive in a browser. Its unit and integration tests via `{{command:theshop-test}} {feature}` are the appropriate gate.
```
...
**Next step:** {the specific fix — e.g. "add `data-testid=\"category-row-delete\"` to ManageCategories.razor via {{command:theshop-implement}}", then re-run `{{command:theshop-e2e}} {feature}`}
```
