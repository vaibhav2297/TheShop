...
Deliver four layers and database migration through specialists. XML documentation remains manual: `{{command:theshop-document}}`. Run `.sdd/scripts/format-changes.ps1` before final verification; repeat affected build checks.
...
> "Please provide a feature name. Usage: `{{command:theshop-implement}} <feature-name>` — for example, `{{command:theshop-implement}} add-to-cart`. The feature name must match an existing plan at `.specs/{feature_name}/plan.md`."
...
> "I couldn't find a plan at `.specs/{arguments}/plan.md`. Implementation works from a plan — please run `{{command:theshop-plan}} {arguments}` first."
...
> "Heads up: the plan for `{arguments}` isn't resolved yet (Status: Draft / {N} open question(s) in Section 11). Building on unresolved questions risks rework. Run `{{command:theshop-resolve}} {arguments}` first, or reply `proceed` to build on the logged assumptions as-is."

Proceed only on the user's explicit go-ahead. This mirrors how `{{command:theshop-plan}}` warns when a spec is still `Draft` — it's a soft gate, not a halt. **If the user says proceed, the waiver gets recorded**: when you update `.specs/{arguments}/status.md` at the end, the Implement row's Gate cell reads `⚠️ waived: plan Draft, {N} open question(s)` instead of a plain pass. Skipped gates are visible, never silent.
...
> **XML documentation is not part of this command.** Documenting the code is a separate, manually-run step. Do **not** invoke `shop-code-documenter` from here — point the user at `{{command:theshop-document}}` in the Next steps instead.
...
2. Run `{{command:theshop-test}} {arguments}` to generate and execute tests.
3. Run `{{command:theshop-e2e}} {arguments}` for browser acceptance proof (user-facing features).
4. Run `{{command:theshop-review}} {arguments}` for parallel quality + security review.
5. Run `{{command:theshop-document}}` manually to add XML doc comments once the code is final.
```
...
Answer the question, then re-invoke `{{command:theshop-implement}} {arguments}`. The completed phases' output is preserved — the next run continues from where this one stopped.
```
...
Resolve the failure manually (read the agent's full output above for context), then re-invoke `{{command:theshop-implement}} {arguments}` to continue from the failing phase, or run the specific layer agent directly if you only need to re-do one step.
```
