...
Every feature carries a `.specs/{feature_name}/status.md` — a one-glance view of where it sits in the SDD pipeline **and the ledger of every verification-gate outcome**. This skill **creates** it; each later step (`{{command:theshop-clarify}}`, `{{command:theshop-plan}}`, `{{command:theshop-resolve}}`, `{{command:theshop-implement}}`, `{{command:theshop-test}}`, `{{command:theshop-verify}}`, `{{command:theshop-review}}`, `{{command:theshop-document}}`) **updates its own row** plus the **Last updated** and **Next step** lines. Use this exact structure:
...
**Next step:** `{{command:theshop-clarify}} {feature_name}`
```
...
1. **Read the tracker on entry.** Before doing anything, check the upstream rows are in the state you expect (e.g. `{{command:theshop-implement}}` expects Plan `Resolved`). If they aren't, warn the user; if the user says proceed, record `⚠️ waived: {reason}` in your own Gate cell so the skip stays visible downstream.
...
|---|---|---|
| 1. Spec | `{{command:theshop-spec}}` → `{{command:theshop-clarify}}` | `Draft` → `Confirmed` |
| 2. Plan | `{{command:theshop-plan}}` → `{{command:theshop-resolve}}` | `Draft` → `Resolved` |
| 3. Implement | `{{command:theshop-implement}}` | `Pending` → `Done` |
| 4. Test | `{{command:theshop-test}}` | `Pending` → `Passing` / `Failing` |
| 5. Verify | `{{command:theshop-verify}}` | `Pending` → `Verified` / `Skipped` |
| 6. Review | `{{command:theshop-review}}` | `Pending` → `Approved` / `Changes requested` |
| 7. Document | `{{command:theshop-document}}` | `Pending` → `Done` |
