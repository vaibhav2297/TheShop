# Preserve phase 5 separately from the original Claude-only migration baseline.
[CmdletBinding()]
param(
...
$checkpoint=Resolve-WorkspacePath $root '.sdd/baseline/caveman'
$inventoryPath=Join-Path $checkpoint 'inventory.json'
$archivePath=Join-Path $checkpoint 'phase5.zip'
$sealPath=Join-Path $checkpoint 'candidate.json'
...
if ($Action -eq 'Capture') {
    if ((Test-Path $inventoryPath) -or (Test-Path $archivePath)) { throw 'Caveman baseline exists; never recapture over it.' }
    ...
    Write-Output "[caveman-checkpoint] captured $($paths.Count) files; phase 5 restore point ready."
    exit 0
}
if (-not (Test-Path $inventoryPath)) { throw 'Capture the phase 5 baseline first.' }
...
if ($Action -eq 'Seal') {
    ...
    Write-Output '[caveman-checkpoint] candidate sealed for conflict-safe rollback.'
    exit 0
}
if ($Action -eq 'Check') {
    ...
    Write-Output "[caveman-checkpoint] archive verified; $($inventory.protectedFiles.Count) protected files unchanged."
    exit 0
}
...
Write-Output "[caveman-checkpoint] restore $($restore.Count) phase 5 files; remove $($remove.Count) Caveman-owned files."
...
Write-Output '[caveman-checkpoint] restored phase 5; unrelated files and migration evidence retained.'
