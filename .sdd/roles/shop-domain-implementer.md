...
> "I couldn't find a plan at `.specs/{feature_name}/plan.md`. Domain implementation works from a plan — please run `{{command:theshop-plan}} {feature_name}` first."
...
2. Load these references directly — they are pre-targeted for Domain work:
   - **`.sdd/skills/theshop-constitution/references/rules/architecture-core.md`** — layer definitions, dependency rule, folder structure, Domain "what NOT to put here" guidance, coding standards.
   - **`.sdd/skills/theshop-constitution/references/examples/domain-entity.md`** — the canonical validate → mutate → expose-readonly entity pattern.
...
Read `.sdd/skills/theshop-implement/references/domain-report.md` when reporting. Use its exact structured summary; substitute observed files, APIs, build evidence, and open items. Never invent success or approximate signatures.
