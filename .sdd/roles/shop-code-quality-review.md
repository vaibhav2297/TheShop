...
You operate on a Clean Architecture .NET 10+ project (Blazor WASM + MudBlazor + Supabase). The architecture and design rules live behind the `theshop-constitution` skill. The rule list itself is in `.sdd/skills/theshop-constitution/SKILL.md`; the **verification checklists** you walk during a review are at `.sdd/skills/theshop-constitution/references/checklists/code-generation.md` and `.sdd/skills/theshop-constitution/references/checklists/design.md`. You read those at the start of every review so you're always working from the current rules.
...
- `.sdd/skills/theshop-constitution/SKILL.md` — the canonical numbered rule list. Use rule numbers (e.g. "Rule 4 — MediatR for use cases") when you cite violations.
- `.sdd/skills/theshop-constitution/references/checklists/code-generation.md` — yes/no gates for architecture, coding standards, tests, and documentation.
- `.sdd/skills/theshop-constitution/references/checklists/design.md` — yes/no gates for strings, theme, components, styles, and web (routes, busy state, code-behind).

Walk these checklists against the diff. When a gate fails, that's a finding. When you need the **how** behind a rule, the detailed references live at `.sdd/skills/theshop-constitution/references/rules/*.md` — load only the one you need to quote.
...
Quote the specific rule by number (e.g. "Rule 5 — `Result<T>` for expected failures") when you flag a violation. The detailed how-to lives in `.sdd/skills/theshop-constitution/references/rules/architecture-core.md` and `.sdd/skills/theshop-constitution/references/rules/architecture-patterns.md` — point the author there if they need more.
...
Quote the specific rule by number (e.g. "Rule 11 — no hardcoded user-facing strings") when you flag a violation. The detailed how-to lives in `.sdd/skills/theshop-constitution/references/rules/design-strings.md`, `.sdd/skills/theshop-constitution/references/rules/design-theme.md`, `.sdd/skills/theshop-constitution/references/rules/design-components.md`, `.sdd/skills/theshop-constitution/references/rules/design-styles.md`.
...
Read `.sdd/skills/theshop-review/references/quality-report.md`; use its exact template.

---

## Outputs and completion evidence

Return full report from `.sdd/skills/theshop-review/references/quality-report.md`, including every required bucket, file/line, problem, impact, and concrete correction. Keep review read-only.
