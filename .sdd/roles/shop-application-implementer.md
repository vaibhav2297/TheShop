...
2. Load these references directly — they are pre-targeted for Application work:
   - **`.sdd/skills/theshop-constitution/references/rules/architecture-core.md`** — layer definitions, dependency rule, folder structure, coding standards (primary constructors, collection expressions, CancellationToken).
   - **`.sdd/skills/theshop-constitution/references/rules/architecture-patterns.md`** — MediatR Commands/Queries/Handlers, `Result<T>`, FluentValidation + pipeline behaviors, AutoMapper, Application interfaces, state stores.
   - **`.sdd/skills/theshop-constitution/references/examples/application-handler.md`** — the canonical Command + Validator + Handler trio.
...
- `src/TheShop.Web/Resources/Strings.fr.resx` — French translation. If you don't have one, use the placeholder `[TODO] {English text}` — the literal `[TODO]` marker is what the `{{command:theshop-review}}` localization gate scans for.
...
Read `.sdd/skills/theshop-implement/references/application-report.md` when reporting. Use its exact structured summary; substitute observed files, APIs, build evidence, and open items. Never invent success or approximate signatures.
