...
2. Load these references directly — they are pre-targeted for Infrastructure work:
   - **`.sdd/skills/theshop-constitution/references/rules/architecture-core.md`** — layer definitions, dependency rule, folder structure, coding standards.
   - **`.sdd/skills/theshop-constitution/references/rules/architecture-patterns.md`** — Application interface declaration, repository pattern, Stripe/Resend adapter pattern, DI registration conventions.
   - **`.sdd/skills/theshop-constitution/references/rules/architecture-admin.md`** — RLS as the only real security boundary, RLS policy examples, role-based access. Mandatory if the feature touches admin tables.
   - **`.sdd/skills/theshop-constitution/references/examples/infrastructure-repository.md`** — canonical Record + Mapper + Repository trio.
...
- `{{tool:claude_ai_Supabase.list_tables}}` — confirm which tables exist.
- `{{tool:claude_ai_Supabase.list_migrations}}` — see what migrations have been applied.
...
Use `{{tool:claude_ai_Supabase.apply_migration}}` for schema changes. Pass:
...
After applying, call `{{tool:claude_ai_Supabase.get_advisors}}` (lint type) and report any new warnings.
...
Read `.sdd/skills/theshop-implement/references/infra-report.md` when reporting. Use its exact structured summary; substitute observed files, APIs, build evidence, and open items. Never invent success or approximate signatures.
